...
Think: Bookworm + Candy Crush + Adventure.# Alphabet Adventure
...
**Phase 1 — Prototype (in progress, pivoted twice)**
...
letters to spell real words. That in turn was replaced with the
current mechanic, Word-Swap: tap/swipe two adjacent tiles to swap
them, and the whole board is scanned for any word(s) the swap created
— true Candy Crush input, just checking for real words instead of
matching identical letters. This is the confirmed core loop going
forward. UI polish (phone-sized bordered panel, shuffle, swap
feedback) is landing incrementally within this phase — see
`update.md`.
